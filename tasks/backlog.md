...
### 既存2フックの `timeout` が秒単位で桁違いになっている

`settings.json.template` の `PreToolUse` 2エントリが `5000` と `10000`（＝1.4時間と2.8時間）。単位は秒であることが公式ドキュメントで確認済み。

**決めること**: 妥当な秒数へ直すか、現状のまま（実質無制限）にするか。
単純に短くすると、`guard-dangerous-bash.py` がタイムアウトした際の挙動次第では
**危険コマンドのブロックが黙って外れる**方向に倒れうる。先にタイムアウト時の
挙動（fail-open か fail-close か）を確認する必要がある。

**着手条件**: フックのタイムアウト時の挙動を確認したとき。
