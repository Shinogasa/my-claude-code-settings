...
- [ ] **Step 2:** 代替案A「旧ruleへ統合」、B「常時ruleだけ」、旧コード参加の限定併存を却下理由付きで確認する。別々の回数上限と全件保存はADR 0012で未決定のため、ここで却下済みと書かない。`docs/adr/README.md` の0012行も確認する。
- [ ] **Step 3:** ユーザーが承認したADRのstatusを `accepted` にする。判断が変われば新ADRで上書きする。承認前にTask 2以降を実行しない。
