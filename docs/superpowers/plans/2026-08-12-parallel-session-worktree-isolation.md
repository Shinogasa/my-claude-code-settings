...
while [ $# -gt 0 ]; do
  case "$1" in
    # shift 2 は残り引数が1つのとき失敗し、同じ分岐に入り続けて無限ループになる。
    # 1つずつ落とすことで、値の無い --self-pid でも必ず抜ける。
    --self-pid) shift; self_pid="${1:-}"; [ $# -gt 0 ] && shift ;;
    *)          shift ;;
  esac
done
...
`timeout` の単位は秒（公式ドキュメントで確認済み）。10 にしているのは、
`lsof` をセッション数ぶん呼ぶため。実測では 7 プロセスで 326ms であり、
それに対して 10 秒あれば十分な余裕がある。プロセス数に比例して伸びる点は
変わらないが、桁が変わるほどの規模は想定しない。
